      ...
      .forEach((post) => {
        ...
        li.innerHTML = `
          ...
          <div class="post-card__actions">
            <a href="/${encodeURIComponent(post.slug)}" target="_blank" rel="noopener">View</a>
            <button type="button" data-action="edit" data-id="${post.id}">Edit</button>
            <button type="button" class="ghost" data-action="delete" data-id="${post.id}">Delete</button>
          </div>
        `;
        postsList.appendChild(li);
      });
  } catch (err) {
    postsList.innerHTML = `<li>Error: ${err.message}</li>`;
  }
}
...
postsList?.addEventListener('click', async (event) => {
  ...
  if (action === 'delete') {
    ...
    try {
      ...
      setStatus(postStatus, 'Post deleted.', true);
      resetForm();
      await loadPosts();
    } catch (err) {
      setStatus(postStatus, err.message);
...
postForm?.addEventListener('submit', async (event) => {
  ...
  setStatus(postStatus, 'Saving...');
  try {
    await apiFetch(endpoint, { method, body: JSON.stringify(payload) });
    setStatus(postStatus, 'Post saved.', true);
    resetForm();
    await loadPosts();
  } catch (err) {
    setStatus(postStatus, err.message);
